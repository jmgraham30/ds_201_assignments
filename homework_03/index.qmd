...
b. Use `pivot_longer` to combine all the pollutant measurements from you new data frame `madrid_pollution_red` into a single column called `poll_measure` and set `names_to = "pollutant"`. Store the result in a new variable named `madrid_pollution_red_long`.
...
c. Now that you have your new variable `madrid_pollution_red_long` run the following command to produce a plot:
