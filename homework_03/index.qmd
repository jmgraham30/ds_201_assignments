...
This homework will involve the use of three data sets so you should begin you Quarto notebook by including and running the follow R code chunk that loads the necessary packages and imports the data:
...
* The avocado prices data set stored as `avocado_prices` comes from the [Hass Avocado Board website](http://www.hassavocadoboard.com/).
...
* The Madrid pollution data set stored as `madrid_pollution` comes from [Kaggle](https://www.kaggle.com/decide-soluciones/air-quality-madrid). This data was collected under the instructions from Madrid's City Council and is publicly available on their website. This data consists of measurements from vehcile inspection stations, the list of possible measurements and their explanations are given by the website:
...
d. Use an appropriate command(s) to compute the mean for the `average_price` variables grouped by region and filter rows with the regions with the top ten mean `average_price` values.
